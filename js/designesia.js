...
// Обробник кліку для якорних посилань в меню
$("a.menu-item").on("click", function(evn) {
    // Закриваємо меню
    $('header').removeClass('menu-open');
    $('header').css('height', 'auto');
    mobile_menu_show = 0;
    $('#menu-btn').removeClass("menu-open"); // Також оновлюємо стан кнопки меню

    // Виконуємо плавний скролінг (якщо це внутрішній якір)
    if (this.hash !== "") {
        evn.preventDefault();
        $('html, body').animate({
            scrollTop: $(this.hash).offset().top
        }, 800, function(){
            // Додавання #hash до URL після завершення анімації (опціонально)
